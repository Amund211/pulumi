...
// rewriteStateMigrationSuccessor applies a successor map after finalStateMigrationSuccessors has collapsed every
// chain and rejected cycles.
...
// different object (or a component) its successor. A successor mapping may change a resource's logical Pulumi identity
// (its URN and type), but a migration only rewrites checkpoint state; it does not ask a provider to import, read, or
// replace the underlying object. A custom resource's physical ID, provider and extension identity, ownership,
// PendingReplacement, and Taint must therefore follow its final successor after callback chaining. Components do not
// have physical or provider identity and may fold freely. Changing these fields directly could orphan the predecessor
// or bind the successor to an unverified object or provider configuration.
...
// to it, and no state may introduce structural references that cannot be resolved. ResourceReference values in
// inputs and outputs are not structural dependencies and may legitimately identify resources in another stack.
