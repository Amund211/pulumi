...
func TestStateMigrationEventJSONRoundTripAndDisplay(t *testing.T) {
	t.Parallel()
	...
	assert.Equal(t, "    component: state migrated: childA → childB\n",
		RenderDiffEvent(event, 0, nil, Options{Color: colors.Never}))
