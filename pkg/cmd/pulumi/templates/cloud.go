...
// getRegistryTemplates runs one registry listing with the options its scheduler chose, reporting
// results into f; which fetch asks for what lives in [newImpl].
func (s *Source) getRegistryTemplates(
	ctx context.Context, f *fetch, r registry.Registry, templateName string,
	opts registry.ListTemplatesOptions,
) {
	// Browsing passes no name, so name resolution is inert and both fetches fall through to the
	// listing below. Only a named lookup, which runs as a single fetch, resolves anything here.
	...
	if urlInfo != nil && urlInfo.Version() != nil {
		s.getRegistryTemplateByVersion(ctx, f, r, urlInfo)
		return
	}
	...
	for page, err := range r.ListTemplates(ctx, opts) {
		...
		// The totals describe every organization the caller belongs to, not the page, so any
		// listing can report them.
		s.observeVcsTemplateSources(page)
...
func (s *Source) getRegistryTemplateByVersion(
	...
	f.addTemplate(registryTemplate{template, r, s})
}
...
func (s *Source) getOrgTemplates(ctx context.Context, f *fetch, templateName string, e env.Env) {
	cwd, err := os.Getwd()
	if err != nil {
