...
// same steps: no create, no delete, and the child keeps its ID and protection.
...
// TestStateMigrationAfterRefresh verifies that `pulumi up --refresh` rebuilds the journal base before applying a
// state migration in the update phase. The renamed resource must retain the identity of the refreshed state.
...
func TestStateMigrationErrors(t *testing.T) {
	...
	t.Run("unknown returned state field", func(t *testing.T) {
		t.Parallel()
		...
		env.migrations = func(t *testing.T, callbacks *deploytest.CallbackServer) []*pulumirpc.Callback {
			callback, err := callbacks.Allocate(func(request []byte) (proto.Message, error) {
				...
				resources[0]["protected"] = true
				newState, err := json.Marshal(resources)
				if err != nil {
					return nil, fmt.Errorf("marshaling new state: %w", err)
				}
				return &pulumirpc.StateMigrationResponse{NewState: newState}, nil
			})
			require.NoError(t, err)
			return []*pulumirpc.Callback{callback}
		}
...
// TestStateMigrationUpdatePlan verifies that an applied update plan cannot authorize a checkpoint rewrite. No-op
// migrations remain safe so callbacks can stay attached after every stack has migrated.
